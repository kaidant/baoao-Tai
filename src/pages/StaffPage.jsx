...
import {
  LayoutDashboard, Users2, FileText, ReceiptText, FolderKanban,
  UserSquare2, ClipboardList, BarChart3, LogOut, Plus, Search, X, Pencil, Trash2,
} from 'lucide-react'
    ...
    <aside style={{ width: 158, background: '#1e3a5f', flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
      ...
      <nav style={{ flex: 1, paddingTop: 6 }}>
        {NAV.map(({ id, label, icon: Icon }) => {
          const isActive = id === 'nhan-su'
          return (
            <button key={id}
              onClick={() => {
                if (id === 'du-an' || id === 'nghiem-thu' || id === 'yeu-cau') onSwitch(id)
              }}
              style={{
...
function StaffModal({ initial, onClose, onSaved }) {
  ...
  const [form, setForm] = useState(isEdit ? {
    ...
    leaveDate: initial.leaveDate || '',
    poGroup:   initial.poGroup   || '',
    notes:     initial.notes     || '',
  } : empty)
  ...
  async function handleSubmit() {
    ...
    if (!form.type) { setError('Vui lòng chọn loại nhân sự'); return }
      ...
      <div style={{ background: '#fff', borderRadius: 12, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>
        ...
        <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            ...
            <div style={grid2}>
              <div>
                <label style={lbl}>Mã nhân sự</label>
                <input value={form.code} onChange={e => set('code', e.target.value)}
                  placeholder="Auto nếu để trống" style={inp} />
              </div>
              <div>
                <label style={lbl}>Họ và tên <span style={{ color: '#ef4444' }}>*</span></label>
            ...
            <div style={{ ...grid2, marginTop: 10 }}>
              <div>
                <label style={lbl}>Loại nhân sự <span style={{ color: '#ef4444' }}>*</span></label>
            ...
            <div style={{ ...grid2, marginTop: 10 }}>
              <div>
                <label style={lbl}>Ngày vào</label>
                <input type="date" value={form.joinDate} onChange={e => set('joinDate', e.target.value)} style={inp} />
              </div>
              <div>
                <label style={lbl}>Ngày nghỉ (nếu có)</label>
                <input type="date" value={form.leaveDate} onChange={e => set('leaveDate', e.target.value)} style={inp} />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {[0,1,2,3,4,5,6].map(n => (
                <button key={n} onClick={() => set('level', n)} style={{
                  width: 36, height: 36, borderRadius: '50%', fontSize: 14, fontWeight: 600,
                  border: form.level === n ? 'none' : '1.5px solid #d1d5db',
                  background: form.level === n ? '#2563eb' : '#fff',
                  color: form.level === n ? '#fff' : '#6b7280',
                  cursor: 'pointer',
                }}>
                  {n}
                </button>
          ...
          <div>
            ...
            <div style={grid2}>
              <div>
                ...
                <input value={form.phone} onChange={e => set('phone', e.target.value)}
                  placeholder="09xxxxxxxx" style={inp} />
              </div>
              <div>
            ...
            <div style={{ ...grid2, marginTop: 10 }}>
              <div>
                <label style={lbl}>Năm sinh</label>
                <input value={form.birthYear} onChange={e => set('birthYear', e.target.value)}
                  placeholder="2002" style={inp} />
              </div>
              <div>
            ...
            <div style={{ ...grid2, marginTop: 10 }}>
              <div>
                <label style={lbl}>Trường học</label>
                <input value={form.school} onChange={e => set('school', e.target.value)}
                  placeholder="Đại học Bách Khoa..." style={inp} />
              </div>
              <div>
                <label style={lbl}>Chuyên ngành</label>
                <input value={form.major} onChange={e => set('major', e.target.value)}
                  placeholder="Cơ Điện Tử..." style={inp} />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ marginBottom: 10 }}>
              <label style={lbl}>Nhóm PO</label>
              <input value={form.poGroup} onChange={e => set('poGroup', e.target.value)}
                placeholder="Tên kỹ sư PO phụ trách..." style={inp} />
            </div>
            <div>
              <label style={lbl}>Ghi chú</label>
              <textarea value={form.notes} onChange={e => set('notes', e.target.value)}
                rows={3} placeholder="Thông tin bổ sung..."
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, padding: '14px 20px', borderTop: '1px solid #f3f4f6' }}>
          <button onClick={onClose} style={{ padding: '8px 20px', background: '#fff', color: '#374151', border: '1px solid #d1d5db', borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
            Hủy
          </button>
...
export default function StaffPage({ onSwitch }) {
  ...
  const stats = TYPES.map(t => ({
    type: t,
    count: staff.filter(s => s.type === t && s.status !== 'Đã nghỉ').length,
  }))
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '14px 24px', background: '#fff', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 1 }}>Quản lý toàn bộ nhân sự Torotic</div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, padding: '14px 24px' }}>
          {stats.map(({ type, count }) => (
            <div key={type} style={{
              ...
              borderLeft: `4px solid ${CARD_BORDER[type]}`,
        ...
        <div style={{ padding: '0 24px 12px', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)}
            style={{ padding: '7px 10px', fontSize: 12, border: '1px solid #e5e7eb', borderRadius: 6, outline: 'none', cursor: 'pointer' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #f3f4f6' }}>
                {['Mã NS','Họ tên','Loại','Lv','Trạng thái','Ngày vào','Giờ chuẩn/tháng','Nhóm PO','Tài khoản',''].map(h => (
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={10} style={{ padding: 40, textAlign: 'center', color: '#9ca3af', fontSize: 13 }}>Không có nhân sự nào</td></tr>
              ) : filtered.map(s => {
                ...
                return (
                  <tr key={s.id} style={{ borderBottom: '1px solid #f9fafb' }}
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      <span style={{ padding: '3px 8px', borderRadius: 20, fontSize: 11.5, fontWeight: 600, background: ts.bg, color: ts.color, border: `1px solid ${ts.border}` }}>
                        {s.type}
                      </span>
                    </td>
                    <td style={{ padding: '10px 14px' }}>
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      <span style={{
                        ...
                        background: s.status === 'Đang làm' ? '#f0fdf4' : s.status === 'Thử việc' ? '#fffbeb' : '#f9fafb',
                        color:      s.status === 'Đang làm' ? '#16a34a' : s.status === 'Thử việc' ? '#d97706' : '#9ca3af',
                      }}>{s.status}</span>
                    </td>
                    ...
                    <td style={{ padding: '10px 14px', color: s.poGroup ? '#2563eb' : '#9ca3af', fontSize: 12, fontWeight: s.poGroup ? 500 : 400 }}>
                      {s.poGroup || '—'}
                    </td>
                    <td style={{ padding: '10px 14px', color: '#9ca3af', fontSize: 12 }}>Chưa gắn</td>
                    <td style={{ padding: '10px 14px' }}>
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button onClick={() => openEdit(s)} style={{ padding: '4px 10px', background: '#eff6ff', color: '#2563eb', border: 'none', borderRadius: 5, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                          Sửa
                        </button>
                        <button onClick={() => handleDelete(s)} style={{ padding: '4px 10px', background: '#fef2f2', color: '#dc2626', border: 'none', borderRadius: 5, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                          Xoá
                        </button>
                      </div>
                    </td>
