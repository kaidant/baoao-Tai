...
/* ── SIDEBAR ───────────────────────────────────────────── */
function Sidebar({ active, setActive, onSwitch }) {
  return (
    <aside style={{ width: 158, background: '#1e3a5f', flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
      ...
      <nav style={{ flex: 1, paddingTop: 6 }}>
        {NAV.map(({ id, label, icon: Icon }) => {
          ...
          return (
            <button key={id}
              onClick={() => { setActive(id); if (id === 'nghiem-thu') onSwitch() }}
              style={{
  ...
  return (
    <div style={{ width: 300, flexShrink: 0, display: 'flex', flexDirection: 'column', background: '#fff', borderRight: '1px solid #e5e7eb' }}>
      ...
      <div style={{ padding: '10px 10px 6px' }}>
        ...
        <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
          {TABS.map(t => (
            <button key={t} onClick={() => setTab(t)} style={{
              padding: '3px 8px', fontSize: 11.5, fontWeight: tab === t ? 600 : 400,
        ...
        {search && (
          <button onClick={() => setSearch('')} style={{ fontSize: 11, color: '#9ca3af', background: 'none', border: 'none', cursor: 'pointer', marginTop: 4 }}>
            Hủy
          </button>
        )}
      </div>
...
function DetailPanel({ project }) {
  ...
  const dropZone = (type, ref, accept) => ({
    ...
    style: {
      border: `2px dashed ${dragOver === type ? '#2563eb' : '#e5e7eb'}`,
      borderRadius: 8, padding: '20px', textAlign: 'center',
      cursor: 'pointer', background: dragOver === type ? '#eff6ff' : '#fafafa',
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 600, color: project.status === 'Hoàn thành' ? '#16a34a' : project.status === 'Tạm dừng' ? '#d97706' : '#2563eb' }}>
              {project.status}
            </span>
          </div>
          <div style={{ display: 'flex', gap: 20 }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: '#2563eb' }}>
              <CircleUser size={13} /> {project.assignee}
            </span>
        ...
        {/* Error */}
        {error && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fef2f2', border: '1px solid #fecaca', color: '#dc2626', padding: '10px 14px', borderRadius: 8, fontSize: 12 }}>
            <AlertCircle size={14} /> {error}
          </div>
        )}
        {submitted && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#f0fdf4', border: '1px solid #bbf7d0', color: '#16a34a', padding: '10px 14px', borderRadius: 8, fontSize: 12 }}>
            <CheckCircle2 size={14} /> Báo cáo đã được nộp thành công!
          </div>
        )}
        ...
        <div style={card}>
          ...
          {!submitted && (
            <div {...dropZone('img', imgRef, 'image/*')}>
              {uploading ? <Loader2 size={18} style={{ margin: '0 auto', color: '#3b82f6', animation: 'spin 1s linear infinite' }} />
                : <Upload size={18} style={{ margin: '0 auto', color: '#d1d5db' }} />}
              <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 6 }}>
                Kéo thả hoặc <span style={{ color: '#2563eb', fontWeight: 500 }}>chọn ảnh</span>
              </div>
        ...
        <div style={card}>
          ...
          {!submitted && (
            <div {...dropZone('vid', vidRef, 'video/*')}>
              <Upload size={18} style={{ margin: '0 auto', color: '#d1d5db' }} />
              <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 6 }}>
                Kéo thả hoặc <span style={{ color: '#7c3aed', fontWeight: 500 }}>chọn video</span>
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {videoFiles.map(f => (
                <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#f5f3ff', borderRadius: 7, padding: '8px 12px' }}>
                  ...
                  {!submitted && (
                    <button onClick={() => handleDelete(f.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#d1d5db', display: 'flex', alignItems: 'center' }}>
                      <Trash2 size={13} />
                    </button>
                  )}
                </div>
        ...
        <div style={card}>
          ...
          <textarea
            ...
            onBlur={() => project && !submitted && saveNotes(project.id, notes)}
            disabled={submitted}
            placeholder="Nhập mô tả, ghi chú hoặc nội dung báo cáo..."
            rows={4}
            style={{
              width: '100%', padding: '10px 12px', fontSize: 13,
              border: '1px solid #e5e7eb', borderRadius: 8, resize: 'none',
              outline: 'none', background: '#fafafa', color: '#374151',
              fontFamily: 'inherit', opacity: submitted ? 0.6 : 1,
            }}
          />
        ...
        {!submitted && (
          <button onClick={handleSubmit}
            disabled={loading || (imageFiles.length === 0 && videoFiles.length === 0)}
            style={{
              width: '100%', padding: '12px', display: 'flex', alignItems: 'center',
              justifyContent: 'center', gap: 8, background: '#2563eb', color: '#fff',
              border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 700,
              cursor: loading || (imageFiles.length === 0 && videoFiles.length === 0) ? 'not-allowed' : 'pointer',
              opacity: loading || (imageFiles.length === 0 && videoFiles.length === 0) ? 0.5 : 1,
            }}>
...
const badge = (bg, color) => ({
  fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: bg, color,
})

/* ── MAIN ───────────────────────────────────────────────── */
export default function SubmitPage({ onSwitch }) {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      <Sidebar active={active} setActive={setActive} onSwitch={onSwitch} />
